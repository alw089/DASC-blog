...
library(ggplot2)
ggplot(data =dat)+
  geom_bar(mapping = aes(y = age_group, x = injury_mechanism ))+
...
```{r}
ggplot(data = dat2)+
  geom_point(mapping = aes(y = diagnosed, x = severity ))+
   ylab("number of diagnoses") +
    xlab("level of severity") +
...
ggplot(data = dat2)+
  geom_point(mapping = aes(y = diagnosed, x = year ))
