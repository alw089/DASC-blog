...
# Question 1
 Which countries in North America currently have the most plants in danger?
...
q1<-filter(dat4, continent %in% c( "North America"))%>%
  ...
  count(threat_type)%>%
  filter(n>1)%>%
  ggplot(aes(x = country, fill = threat_type))+
    geom_bar()
q1
...
```
Of the plants that became extinct in 2000-2020 due to pollution, the majority were last seen in America. 16 were from the United States, 2 from Mexico. This insinuates that America has a much larger pollution problem at hand and it is detrimental to their plant species. 
...
# Conclusion
This data set provides very valuable information about the future of plants. This becomes a human problem because we are so dependent on plants to produce the oxygen in our atmosphere.