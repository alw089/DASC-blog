...
ggplot(data = dat2)+
  geom_boxplot(mapping = aes(y = diagnosed, x = factor(year) , color = service))+
  ggtitle("Number of diagnoses per branch memebrship compared to the year")+
...
graph1<-filter(dat1, type %in% c("Deaths" ))%>%
  ...
  geom_boxplot(mapping = aes(y= number_est, x= injury_mechanism, color = injury_mechanism))+
  theme(axis.text.x = element_text(angle =- 45, hjust = 0))+
...
graph2<-filter(dat, type %in% c("Deaths" ))%>%
  ggplot(aes(count = type ,x= injury_mechanism, color = injury_mechanism))+
  geom_boxplot(mapping = aes(count = type, x= injury_mechanism, color = injury_mechanism))+
  theme(axis.text.x = element_text(angle =- 45, hjust = 0))+
